...
Use the repository entrypoint `npm run --silent rokai -- --ready` from `ROKAI_ROOT`. It is a persistent, runtime-owned local-file host transport around the existing `createRokaiHostMediatedSession()` and deterministic policy engine. Codex must pass the exact raw results of the authenticated read tools in a structured `start` request through the repo-owned request helper described below:
...
For approval, send an `approve` request to the same process with the same `runId`, the user's exact `approve`, `approve plan`, or `APPROVE <planId>` text, and fresh account/price reads. Include exchange-info again only when the Ready Mode response reports a metadata-cache miss or expiry; a valid 45-second symbol-metadata cache is sufficient and avoids resending the large permission matrix. If the runtime rejects the approval or changes the plan, stop. The runtime checks plan expiry immediately before releasing the executable payload; an expired plan returns `PLAN_EXPIRED` and requires a fresh deterministic plan and fresh approval. Only the exact payload returned by the runtime may be passed to the sanctioned Binance host tool after approval. Use a `verify` request with the runtime's opaque `submissionId` and fresh read results after the host call. The CLI/runtime never calls Binance itself; it is the deterministic authority between Codex's authenticated MCP reads and any sanctioned host transport.

Never independently calculate or invent quantities, quoteOrderQty, symbols, sides, filters, rounding, plan IDs, preflight results, or order payloads. A normal `$rokai <policy>` invocation must not use `rg`, source inspection, repo-wide search, temporary custom scripts, transport discovery, or repeated Binance tool discovery when the known tools are available. If `npm run --silent rokai -- --ready` or `npm run --silent rokai:request` cannot be used, or the response cannot prove an authoritative plan, provide read-only rule commentary only and stop before approval or execution. Do not fall back to LLM-generated trade planning.

## Ready Mode

Start `npm run --silent rokai -- --ready` from `ROKAI_ROOT` once for every host-mediated workflow, instead of restarting the runtime between requests. Ready Mode emits one small structured startup status containing `transport.requestDirectory`, `transport.capability`, `transport.requestSuffix`, and `transport.responseSuffix`. It confirms that the deterministic modules are loaded and reports the effective `ROKAI_LIVE_EXECUTION` value, then keeps the same process alive for multiple policy runs and approvals. It does not read balances, create a plan, approve anything, or call a Binance tool at startup.
...
Use the repo-owned helper for every host-mediated request. First create a plain local JSON request-input file with the request object only, then run exactly:
...
The helper generates a unique request ID, atomically writes the runtime wire file `<requestId>.request.json` in the reported directory, waits for `<requestId>.response.json`, prints only that JSON response, and leaves the response file available for the active runtime session. Codex should use its normal file-writing capability for the request-input file; it must not construct PowerShell filesystem commands or create a temporary Node/PowerShell proxy.

The exact runtime wire envelope is:

```json
{"capability":"<the ready capability>","requestId":"<same unique id>","request":{"op":"start","policyText":"<the user's exact policy>","settlementAsset":"USDT","reads":{"account":<spot.getAccount result>,"prices":<spot.tickerPrice result>,"exchangeInfo":<spot.exchangeInfo result>}}}
```

The helper input file contains only the nested `request` object. Use these exact operation shapes:
...
```json
{"op":"approve","runId":"<runId>","approval":"approve","reads":{"account":<fresh spot.getAccount result>,"prices":<fresh spot.tickerPrice result>}}
```

```json
{"op":"verify","submissionId":"<submissionId>","reads":{"order":<spot.getOrder result>,"account":<fresh spot.getAccount result>,"prices":<fresh spot.tickerPrice result>}}
```

For `replan`, use `{"op":"replan","runId":"<runId>","settlementAsset":"USDT","reads":{"account":<fresh account>,"prices":<fresh prices>}}`. The runtime consumes each wire request and deletes it. Use the same process capability and directory for `start`, `replan`, `approve`, and `verify`; never put a large payload on a command line or raw stdin. The transport is local-only, has no network listener or named pipe, accepts only structured runtime operations, and rejects missing or incorrect capabilities and reused request IDs.
